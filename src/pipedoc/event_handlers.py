...
from pipedoc.event_system import EventListener, Event
from pipedoc.app_logger import AppLogger, LogContext, get_default_logger
...
class EventHandlerChain:
    ...
    def __init__(
        self,
        handlers: Optional[List[EventListener]] = None,
        ...
        Args:
            ...
            logger: Application logger for error reporting (uses default if None)
        """
        self._handlers = handlers or []
        self._logger = logger or get_default_logger()
        self._context = LogContext(component="EventHandlerChain")

    def add_handler(self, handler: EventListener) -> "EventHandlerChain":
        """Add a handler to the chain (fluent interface)."""
        self._handlers.append(handler)
        return self

    def remove_handler(self, handler: EventListener) -> bool:
...
class FilteringEventListener(EventListener):
    """Event handler that filters events before passing to another handler."""

    def __init__(
        self,
        target_handler: EventListener,
    ...
    def on_connection_event(self, event_data: ConnectionEvent) -> None:
        ...
        if self._format_type == "json":
            ...
            if event_data.metadata:
                metadata_str = ", ".join(
                    f"{k}={v}" for k, v in event_data.metadata.items()
                )
    ...
    def on_connection_event(self, event_data: ConnectionEvent) -> None:
        ...
        if self._format == "json":
            ...
            timestamp_str = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(event_data.timestamp)
            )
            line = (
                f"[{timestamp_str}] {event_data.event_type} {event_data.connection_id}"
            )
