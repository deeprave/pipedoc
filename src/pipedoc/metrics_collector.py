    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        with self._metrics_lock:
            ...
            if self._connection_times:
                avg_connection_time = sum(self._connection_times) / len(
            ...
            if self._queue_wait_times:
                avg_queue_wait_time = sum(self._queue_wait_times) / len(
            ...
            return {
                ...
                "connection_duration_histogram": dict(
                    self._connection_duration_histogram
                ),
            }
    ...
    def on_connection_event(self, event_data: "ConnectionEvent") -> None:
        ...
        with self._metrics_lock:
            ...
            if event_data.connection_event_type == ConnectionEventType.CONNECT_ATTEMPT:
                ...
                event_data.connection_event_type == ConnectionEventType.CONNECT_FAILURE
            ):
                self._failed_connections += 1
                ...
                event_data.connection_event_type == ConnectionEventType.CONNECT_DEQUEUED
            ):
                self._current_queue_depth = max(0, self._current_queue_depth - 1)
                ...
                event_data.connection_event_type == ConnectionEventType.CONNECT_TIMEOUT
            ):
                self._connections_timeout += 1
                self._current_queue_depth = max(0, self._current_queue_depth - 1)
