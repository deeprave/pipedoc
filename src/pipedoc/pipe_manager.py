...
from pipedoc.event_system import EventListener
...
class PipeManager:
    ...
    def __init__(
        ...
        self._worker_pool = WorkerPool(
            max_workers=max_workers, metrics_collector=self._metrics_collector
        )
        ...
        self._log_context = LogContext(component="PipeManager")

        self._logger.info(
            "Enhanced PipeManager initialised with component architecture",
    ...
    def create_named_pipe(self) -> str:
        ...
        with self._state_lock:
            ...
            if pipe_path:
                self._logger.info(
                    "Created named pipe", context=self._log_context, pipe_path=pipe_path
                )
                return pipe_path
            else:
                self._logger.error(
                    "Failed to create named pipe", context=self._log_context
                )
    ...
    def serve_client(self, client_id: int, content: str) -> None:
        ...
        """
        self._logger.debug(
            "Client opening pipe for writing",
            context=self._log_context,
            client_id=client_id,
        )
        ...
        try:
            ...
            with open(pipe_path, "w") as pipe:
                ...
                self._logger.debug(
                    "Client connected, sending content",
                    context=self._log_context,
                    client_id=client_id,
                ...
                self._logger.info(
                    "Client content sent successfully",
                    context=self._log_context,
                    client_id=client_id,
            ...
            self._logger.error(
                "Error serving content to client",
                context=self._log_context,
                client_id=client_id,
                error=str(e),
            )
    ...
    def start_serving(self, content: str) -> None:
        ...
        with self._state_lock:
            ...
            self._logger.info(
                "Initialising always-ready writer", context=self._log_context
            )
            ...
            self._logger.info(
                "Press Ctrl+C to stop the server", context=self._log_context
            )
    ...
    def stop_serving(self) -> None:
        ...
        with self._state_lock:
            ...
            self._logger.info("Server stopped", context=self._log_context)
    ...
    def cleanup(self) -> None:
        ...
        with self._state_lock:
            ...
            self._logger.info(
                "Shutting down thread pool gracefully",
                context=self._log_context,
                active_tasks=active_tasks,
            )
            self._worker_pool.shutdown(wait=True)
            self._logger.info(
                "Thread pool shut down gracefully", context=self._log_context
            )
            ...
            if pipe_path:
                self._logger.info(
                    "Cleaned up pipe", context=self._log_context, pipe_path=pipe_path
                )
