...
class TestWorkerPool:
    ...
    def test_worker_pool_lifecycle_management(self):
        ...
        assert pool.get_capacity() == max_workers, (
            f"Pool capacity should be {max_workers}"
        )
        ...
        for i, future in enumerate(futures):
            ...
            assert result == f"Task {i} completed", (
                f"Task {i} should complete successfully"
            )

        # Assert - Verify all tasks executed
        assert len(task_results) == 5, "All 5 tasks should have executed"
        for i in range(5):
            assert f"Task {i} completed" in task_results, (
                f"Task {i} should be in results"
            )
