...
class TestPipeResource:
    ...
    def test_pipe_resource_creation_and_monitoring(self):
        ...
        assert pipe_resource.get_pipe_path() is None, (
            "Should have no pipe path initially"
        )
        assert not pipe_resource.is_pipe_created(), (
            "Should not have pipe created initially"
        )
        ...
        # Assert - Cleanup
        assert not pipe_resource.is_pipe_created(), (
            "Should indicate pipe is not created after cleanup"
        )
    ...
    @patch("os.open")
    @patch("os.close")
    def test_pipe_resource_connection_monitoring(
        self, mock_close, mock_open, mock_select
    ):
        ...
        try:
            ...
            assert not connection_detected, (
                "Should not detect connection when select returns empty"
            )
            ...
            assert connection_detected, (
                "Should detect connection when select returns ready fd"
            )
    ...
    def test_pipe_resource_path_management(self):
        ...
        try:
            ...
            assert path1 != path2, (
                "Different instances should create different pipe paths"
            )
