...
class TestMetricsCollector:
    ...
    def test_metrics_collector_basic_functionality(self):
        ...
        assert "avg_connection_time" in metrics, (
            "Should calculate average connection time"
        )
        ...
        assert len(metrics["connection_times"]) == 2, (
            "Should preserve individual connection times"
        )
        assert 0.05 in metrics["connection_times"], (
            "Should contain first connection time"
        )
        assert 0.03 in metrics["connection_times"], (
            "Should contain second connection time"
        )
    ...
    def test_metrics_collector_thread_safety(self):
        ...
        assert all("completed" in result for result in results), (
            "All threads should report completion"
        )
    ...
    def test_metrics_collector_reset_functionality(self):
        ...
        assert metrics_before["connection_attempts"] > 0, (
        ...
        assert metrics_after["connection_attempts"] == 0, (
            "Attempts should be reset to 0"
        )
        assert metrics_after["successful_connections"] == 0, (
        ...
        assert metrics_after["avg_connection_time"] == 0.0, (
            "Average time should be reset to 0"
        )
        assert len(metrics_after["connection_times"]) == 0, (
            "Connection times should be empty"
        )
    ...
    def test_metrics_collector_edge_cases(self):
        ...
        assert empty_metrics["success_rate"] == 0.0, (
            "Success rate should be 0 with no attempts"
        )
        assert empty_metrics["avg_connection_time"] == 0.0, (
            "Average time should be 0 with no connections"
        )
        ...
        assert failure_only_metrics["success_rate"] == 0.0, (
            "Success rate should be 0 with only failures"
        )
        assert failure_only_metrics["avg_connection_time"] == 0.0, (
            "Average time should be 0 with no successful connections"
        )
    ...
    def test_queue_metrics_basic_functionality(self):
        ...
        assert metrics["connections_queued"] == 3, (
            "Should track total connections queued"
        )
        assert metrics["connections_timeout"] == 1, "Should track connection timeouts"
        assert metrics["current_queue_depth"] == 0, (
            "Should track current queue depth (3 queued - 2 dequeued - 1 timeout = 0)"
        )
        assert metrics["max_queue_depth"] == 2, (
            "Should track maximum queue depth reached"
        )

        # Assert - Verify queue timing calculations
        assert "avg_queue_wait_time" in metrics, (
            "Should calculate average queue wait time"
        )
    ...
    def test_queue_metrics_reset(self):
        ...
        # Verify data exists
        metrics_before = collector.get_metrics()
        assert metrics_before["connections_queued"] > 0, (
            "Should have queue data before reset"
        )
        ...
        metrics_after = collector.get_metrics()
        assert metrics_after["connections_queued"] == 0, (
            "Queued connections should be reset to 0"
        )
        assert metrics_after["connections_timeout"] == 0, (
            "Timeout connections should be reset to 0"
        )
        assert metrics_after["current_queue_depth"] == 0, (
            "Current queue depth should be reset to 0"
        )
        assert metrics_after["max_queue_depth"] == 0, (
            "Max queue depth should be reset to 0"
        )
        assert metrics_after["avg_queue_wait_time"] == 0.0, (
            "Average wait time should be reset to 0"
        )
        assert len(metrics_after["queue_wait_times"]) == 0, (
            "Queue wait times should be empty"
        )
...
class TestMetricsCollectorEventIntegration:
    ...
    def test_metrics_collector_implements_event_listener(self):
        ...
        assert isinstance(collector, ConnectionEventListener), (
            "Should implement ConnectionEventListener protocol"
        )
        assert hasattr(collector, "on_connection_event"), (
            "Should have on_connection_event method"
        )
    ...
    def test_metrics_collector_event_driven_basic_lifecycle(self):
        ...
        assert metrics["successful_connections"] == 1, (
            "Should count successes from events"
        )
    ...
    def test_metrics_collector_event_driven_queue_metrics(self):
        ...
        metrics = collector.get_metrics()
        assert metrics["connections_queued"] == 1, (
            "Should count queued connections from events"
        )
    ...
    def test_metrics_collector_event_driven_enhanced_statistics(self):
        ...
        assert "connection_duration_histogram" in metrics, (
            "Should provide duration histogram"
        )
    ...
    def test_metrics_collector_event_backwards_compatibility(self):
        ...
        assert metrics["connection_attempts"] == 2, (
            "Should count both traditional and event attempts"
        )
        assert metrics["successful_connections"] == 1, (
            "Should count traditional success"
        )
    ...
    def test_metrics_collector_event_thread_safety(self):
        ...
        assert metrics["connection_attempts"] == expected_total, (
            f"Should record all {expected_total} attempts"
        )
        assert metrics["successful_connections"] == expected_total, (
            f"Should record all {expected_total} successes"
        )
