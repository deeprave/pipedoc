...
This module tests that ConnectionEvent properly inherits from Event
...
from pipedoc.event_system import Event, EventListener, EventLevel
...
class TestEventSystemIntegration:
    ...
    def test_connection_event_data_inherits_from_application_event(self):
        """Test that ConnectionEvent is a proper Event."""
        # Arrange & Act
        ...
        assert isinstance(event_data, Event), "Should be an Event"
    ...
    def test_connection_event_level_mapping(self):
        ...
        for event_type, expected_level in test_cases:
            event_data = ConnectionEvent(
                event_type=event_type, connection_id="test", timestamp=time.time()
            )
            assert event_data.level == expected_level, (
                f"{event_type} should map to {expected_level}"
            )
    ...
    def test_connection_event_listener_extends_event_listener(self):
        ...
        assert isinstance(listener, ConnectionEventListener), (
            "Should implement ConnectionEventListener"
        )
        # Note: Can't easily test EventListener isinstance due to Protocol limitations

        # Test delegation works
        connection_event = ConnectionEvent(
            ConnectionEventType.CONNECT_SUCCESS, "test_conn", time.time()
        )
    ...
    def test_backwards_compatibility_maintained(self):
        ...
        event_data = ConnectionEvent(
            ConnectionEventType.CONNECT_SUCCESS, "backwards_test", time.time()
        )
        ...
        # And it should have Event properties
...
class TestEventSystemDesignPrinciples:
    ...
    def test_liskov_substitution_principle(self):
        """Test that ConnectionEvent can substitute Event."""

        # Arrange
        def process_application_event(event: Event) -> str:
            """Function that expects Event."""
            return f"{event.component}: {event.message}"

        # Act - Should work with ConnectionEvent
        connection_event = ConnectionEvent(
            ConnectionEventType.CONNECT_SUCCESS, "lsp_test", time.time()
        )
    ...
    def test_single_responsibility_principle(self):
        ...
        # Arrange
        connection_event = ConnectionEvent(
            ConnectionEventType.CONNECT_QUEUED, "srp_test", time.time()
        )
